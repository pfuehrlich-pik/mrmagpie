...
#' e.g. "ISIMIP3bv2:IPSL-CM6A-LR:ssp126:1965-2100"
...
calcCollectEnvironmentData_new <- # nolint: object_name_linter.
    function(subtype = "ISIMIP3bv2:IPSL-CM6A-LR:ssp126:1965-2100",
             sar = 20, sel_feat = c("tas", "pr", "lwnet", "rsds", "CO2", "Ks", # nolint: object_name_linter.
                                    "Sf", "w_pwp", "w_fc", "w_sat", "hsg", "wet")) {
  ##### CONFIG ######
  ...
  # read in the individual climate variables, then smooth the dataset with toolAverage and
  # extend the standardize the number of years)
  gcmVariables <- list()
  for (climate_variable in climateVariables) {
    gcmVariables[[climate_variable]] <- calcOutput("GCMClimate_new", aggregate = FALSE,
                                                   subtype = paste(x$version, x$climatemodel, x$scenario,
                                                                   fullSimulationPeriod, climate_variable,
                                                                   "annual_mean", sep = ":"))
    gcmVariables[[climate_variable]] <- toolHoldConstant(gcmVariables[[climate_variable]],
                                                         seq((max(getYears(gcmVariables[[climate_variable]],
  ...
  return(list(x = env,
              weight = weight,
              unit = paste("temperature: Degree Celcius",
